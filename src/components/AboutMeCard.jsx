...
const AboutMeCard = ({ visible, onClose }) => {
  ...
  return (
    // Backdrop that closes the popup when clicked
    <div
      className="fixed inset-0 z-10 bg-black bg-opacity-30"
      onClick={onClose}
    >
      {/* Popup container — stop event bubbling so clicks inside don’t close it */}
      <div
        onClick={(e) => e.stopPropagation()}
        className={`fixed bottom-0 left-0 w-full transition-transform duration-500 bg-white shadow-lg border border-black rounded 
          ...
          <div className="mt-4 p-1 flex flex-col sm:flex-row sm:items-center gap-6">
            ...
            <div className="flex flex-col gap-4 sm:gap-14 max-w-180 w-full">
              <p className="text-black text-base sm:text-2xl leading-5 sm:leading-[2.0rem] mt-3 sm:mt-0 text-left">
                ...
                <br />
                <br />I bring with me a strong sense of curiosity, a genuine
                love of learning, close attention to the smallest details, and a
                real appreciation for teamwork and collaboration.
              </p>
              <div className="mt-4 mb-5 flex flex-col gap-2 sm:flex-row sm:justify-start sm:items-stretch">
                {/* Email + CV horizontally */}
